...
class SecurityOps:
    ...
    def _generate_recommendations(self, audit_results: Dict[str, Any]) -> List[str]:
        ...
        if audit_results["score"] < 70:
            ...
            recommendations.append("🟡 HIGH PRIORITY: Address security vulnerabilities promptly")

        # توصيات محددة
        if any(v["type"] == "dependency" for v in audit_results["vulnerabilities"]):
            recommendations.append("Update vulnerable dependencies using 'poetry update'")

        if any("permissions" in issue["description"].lower() for issue in audit_results["compliance_issues"]):
            recommendations.append("Fix file permissions: chmod 600 on sensitive files")

        if any("default" in issue["description"].lower() for issue in audit_results["compliance_issues"]):
            recommendations.append("Replace default secrets with secure generated values")
    ...
    def generate_security_report(self) -> str:
        ...
        report_path = self.reports_dir / f"security_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
