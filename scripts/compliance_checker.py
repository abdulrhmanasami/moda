...
class ComplianceChecker:
    ...
    def run_full_compliance_check(self) -> Dict[str, Any]:
        ...
        # حساب الامتثال العام
        results['overall_compliance'] = self.calculate_overall_compliance(results['categories'])
        ...
        results['critical_issues'] = self.identify_critical_issues(results['categories'])
    ...
    def check_architecture_compliance(self) -> Dict[str, Any]:
        ...
        existing_dirs = [d.name for d in self.src_path.iterdir() if d.is_dir()] if self.src_path.exists() else []

        dir_compliance = len(set(required_dirs) & set(existing_dirs)) / len(required_dirs) * 100
        ...
        for check_name, check_data in compliance['checks'].items():
            if check_data['score'] < 70:
                compliance['issues'].append(f"Low compliance in {check_name}: {check_data['score']:.1f}%")
    ...
    def check_backend_framework(self) -> Dict[str, Any]:
        ...
        main_files = list(backend_path.glob("**/main.py")) + list(backend_path.glob("**/app.py"))
        if main_files:
            compliance['score'] += 30
        ...
        compliance['details']['code_structure'] = f"{matched_files}/{len(required_files)}"
    ...
    def check_frontend_framework(self) -> Dict[str, Any]:
        ...
        compliance['details']['nextjs_structure'] = f"{len(existing_nextjs)}/{len(nextjs_files)}"
    ...
    def check_code_quality(self) -> Dict[str, Any]:
        ...
        for file_path in python_files:
            try:
                ...
                # فحص طول الأسطر
                for line in lines:
                    if len(line) > self.compliance_rules['code_quality']['line_length_limit']:
                        long_lines += 1
    ...
    def check_test_coverage(self) -> Dict[str, Any]:
        ...
        if src_files:
            ...
            coverage['details']['coverage_ratio'] = f"{len(test_files)}/{len(src_files)}"
    ...
    def check_security_compliance(self) -> Dict[str, Any]:
        ...
        security_score = sum(1 for f in security_files if (self.project_root / f).exists())
    ...
    def identify_violations(self, categories: Dict[str, Dict]) -> List[str]:
        ...
        for category, data in categories.items():
            score = data.get('score', 0)
            if score < 60:
                violations.append(f"Critical: {category} compliance is below 60% ({score:.1f}%)")

            # مخالفات محددة للفئات
            if category == 'architecture':
                if 'issues' in data:
                    violations.extend([f"Architecture: {issue}" for issue in data['issues']])

            elif category == 'security':
                if 'issues' in data.get('checks', {}).get('security', []):
                    violations.extend([f"Security: {issue}" for issue in data['checks']['security']['issues']])
    ...
    def identify_critical_issues(self, categories: Dict[str, Dict]) -> List[str]:
        ...
        for category, data in categories.items():
            ...
            if score < 40:
                critical_issues.append(f"🚨 CRITICAL: {category.title()} compliance is critically low ({score:.1f}%)")

            # مشاكل حرجة محددة
            if category == 'architecture':
                checks = data.get('checks', {})
                if checks.get('directory_structure', {}).get('score', 0) < 30:
                    critical_issues.append("🚨 CRITICAL: Project structure does not match technical specifications")

            if category == 'security':
                security_checks = data.get('checks', {}).get('security', {})
                if security_checks.get('score', 0) < 50:
                    critical_issues.append("🚨 CRITICAL: Security vulnerabilities detected in code")
...
def main():
    ...
    # حفظ النتائج في ملف
    output_file = Path(__file__).parent.parent / "logs" / f"compliance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
